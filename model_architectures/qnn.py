...
class QNN(BaseQuantumModel):
    """
    Quantum Neural Network (QNN)

    Key idea:
        - parameterized quantum circuit acts like a neural network
        - uses a designated readout qubit for prediction
    """
    ...
    def __init__(
        ...
        def circuit(inputs, weights):
            ...
            # Variational "neural network" layers
            for l in range(self.n_layers):

                # Single-qubit rotations (like neurons)
                for i in range(self.n_qubits):
                    qml.RY(weights[l, i, 0], wires=i)
                    qml.RZ(weights[l, i, 1], wires=i)

                # Entanglement (information mixing)
